  ...
  <div>
    <div id="chart-header-bar">
      <div class="resolution-puls-price">
        <div id="serie-legend">24H Volume / ETH, 1D, UniswapDEX</div>
      </div>
      <div>
        <div id="serie-legend">24H Vol: {{ volume.toFixed(4)}}</div>
      </div>
    </div>
    <div id="volume-line-chart"></div>
...
export default {
  ...
  mounted: async function() {
    ...
    let url = `${config.uniswapDexServer}api/histodayvolume?start=${from}`;
    ...
    const chartContainer = document.querySelector("#volume-line-chart");
...
#volume-line-chart > div,
#volume-line-chart > div > table {
  width: 100% !important;
